...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
  ...
  try {
    db_query(
      $pdo,
      "INSERT INTO product (name_product,code_product,price_product,Volume_constraint,Service_time,Location,agent,data_limit_reset,note,category,hide_panel,one_buy_status,limit_ip) VALUES (?,?,?,?,?,?,?,'no_reset',?, ?,'{}','0',?)",
      [$name, $code, (int) ($_POST['price_product'] ?? 0), (int) ($_POST['volume_product'] ?? 0), (int) ($_POST['time_product'] ?? 0), $_POST['namepanel'] ?? '', $_POST['agent_product'] ?? '', $_POST['note_product'] ?? '', $_POST['cetegory_product'] ?? '', (int) ($_POST['limit_ip'] ?? 0)]
    );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
  ...
  if ($pid && $name !== '') {
    try {
      db_query(
        $pdo,
        "UPDATE product SET name_product=?,price_product=?,Volume_constraint=?,Service_time=?,Location=?,agent=?,note=?,category=?,limit_ip=? WHERE id=?",
        [$name, (int) ($_POST['price_product'] ?? 0), (int) ($_POST['volume_product'] ?? 0), (int) ($_POST['time_product'] ?? 0), $_POST['namepanel'] ?? '', $_POST['agent_product'] ?? '', $_POST['note_product'] ?? '', $_POST['cetegory_product'] ?? '', (int) ($_POST['limit_ip'] ?? 0), $pid]
      );
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-grid">
          ...
          <div class="field">
            <label>محدودیت IP هم‌زمان</label>
            <input type="number" name="limit_ip" class="input" value="0" min="0" placeholder="0 = بدون محدودیت">
          </div>
          <div class="field">
    ...
    <form method="POST">
      <div class="modal-body">
        ...
        <div class="form-grid">
          ...
          <div class="field">
            <label>محدودیت IP هم‌زمان</label>
            <input type="number" name="limit_ip" id="edit_limit_ip" class="input" value="0" min="0">
          </div>
          <div class="field">
